...
In 2002, there were 6 states observed at 7 or more locations: CT, FL, MA, NC, NJ, and PA. In 2010, there were 14 states observed at 7 or more locations. 
```{r}
...
brfss_cln%>%
 filter(response =="Excellent")%>%
  ...
  geom_line() +
  xlab('Year') + ylab('State Average Value')+
...
Plot is showing all of the state averages over time in years. No obvious trends in this plot. Not the most informative plot. It might be better to separate by region (ex. Northeast, Southeast, etc.) so that you can pick up on distinct trends. 
```{r}
...
Plot shows that there is not much of a discrepancy between years 2010 and 2006. Responses for Good, Very Good, and Excellent had higher data_value scores than Fair and Poor. 
...
accel%>%
  mutate(activity_sum_min = (accel%>%
                               ...
                               rowSums()))%>%
  select(day_id, activity_sum_min)%>%
  ggplot(aes(x = day_id, y = activity_sum_min)) + geom_point()+geom_line()
```

There are certain days with only 1440 activity recorded. These may be days where accelerometer did not accurately pick up movement/activity data. There are no obvious trends apparent. 
```{r}
